...
twitter_data = spark.read.csv(
    "hdfs://localhost:8020/user/BDprojekt/twitter_*.csv",
    header=True,
...
crypto_data = spark.read.csv(
    "hdfs://localhost:8020/user/BDprojekt/crypto_*.csv",
    header=True,
...
twitter_data.write.partitionBy("time").saveAsTable("twitter_data", mode="overwrite")
crypto_data.write.partitionBy("time").saveAsTable("crypto_data", mode="overwrite")
 
# %%
spark.sql("select * from twitter_data").show()
...
if b"projekt" not in connection.tables():
